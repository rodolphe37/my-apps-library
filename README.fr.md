...
- **Thème clair/sombre** avec détection automatique de l'OS, plus un interrupteur manuel, et un choix de **palette de couleurs** (**Préférences → Palette de couleurs**) : la palette intégrée par défaut, ou toute palette fournie par un plugin, chacune avec sa propre variante claire+sombre. Toute l'interface (contour de sélection, barre latérale, boutons) est stylée par défaut autour du dégradé bleu-violet du logo de l'application (voir [`src/myapps/ui/theme/brand.py`](src/myapps/ui/theme/brand.py)) ; un projet sélectionné est mis en valeur par une bordure de couleur d'accent plutôt qu'un fond plein, pour que ses propres couleurs d'icône/badge restent lisibles.
- **Barre de menus native** avec toutes les actions principales, entièrement utilisable au clavier.
- **Système de plugins**, installation depuis un `.zip` ou un dossier local, activation/désactivation depuis **Plugins → Gérer les plugins…** ; les plugins peuvent contribuer des actions de menu contextuel, des actions de menu, de nouveaux modes d'affichage, des packs d'icônes, des palettes de couleurs et des traductions. **Plugins → Parcourir la Marketplace…** ouvre une application web compagnon de marketplace de plugins dans votre navigateur (code source conservé dans un dépôt privé) ; l'installation elle-même reste uniquement locale (`.zip`/dossier), aucun compte marketplace nécessaire. Seule exception : la mise à jour en un clic — si un plugin installé est aussi publié là-bas, la fenêtre vérifie silencieusement auprès de l'API publique de la marketplace s'il existe une version plus récente et propose un bouton **Mettre à jour** - le seul appel réseau que ce système fait de lui-même.
...
- **Vérification de mise à jour** - au démarrage, vérifie silencieusement la dernière release GitHub par rapport à la version en cours ; s'il y en a une plus récente, une fenêtre affiche la commande de mise à jour exacte pour votre OS. Le seul appel réseau que l'application fait de sa propre initiative, et il échoue silencieusement (pas de message d'erreur) si vous êtes hors-ligne ou que GitHub est inaccessible.
...
- Un plugin peut contribuer :
  ...
  - Des **packs d'icônes** pour les catégories et les projets (`contribute_icon_packs`)
  - Des **palettes de couleurs**, claires + sombres, sélectionnables depuis les Préférences (`contribute_theme_palettes`)
...
Un exemple réel et publié est aussi en ligne sur la marketplace : [Theme & Icons Pack](https://marketplace.rodolphe-augusto.fr/plugins/theme-and-icons-pack) démontre `contribute_icon_packs` et `contribute_theme_palettes` ensemble, avec une fiche complète (description, captures d'écran) rédigée via le CLI `mal-plugin`.

**`Plugins → Parcourir la Marketplace…`** ouvre une application web compagnon de marketplace de plugins (son code source vit dans un dépôt séparé et privé, il ne fait pas partie de ce projet). Elle lit la variable d'environnement `MYAPPS_MARKETPLACE_URL`, avec par défaut la marketplace en ligne à `https://marketplace.rodolphe-augusto.fr` ; ne définissez cette variable vous-même que pour pointer vers une instance de développement locale (`http://localhost:5173`).
